...
% Paper title.
\title{Curvature Enhancement With and Adapted Laplacian Operator For Hybrid Quad/Triangle Meshes}
...
method, this work also presents new applications in sculpting and
...
curvature, whereby it performs a curvature enhancement and shape exaggeration
of the model, creating a family of different versions of the same
object and therefore preserving the detail of the original model and
a realistic appearance.

The curvature enhancement and shape exaggeration can be used in the
sculpting proccess when inflating with a brush since current brushes
that perform inflation end up by losing detail when moving vertices
\cite{Stanculescu2011}, in contrast the presented method allows inflation
of the mesh vertices by moving them towards the reverse curvature
direction, conserving the shape and sharp features of the model.

Interest on meshes composed by triangles and quads has increasing
in recent years due to the flexibility of modeling tools as Blender
3D \cite{blender}. Today many artists manually connect vertices such
that its edition allows a simple way to perform animation processes
and interpolation \cite{Mullen2007}. For these reasons it is very
important to develop an operator that allows working with this type
of mesh immediately, eliminating the need to preprocessing the mesh
to convert to triangles and losing the original design made by users. 
...
as to minimize the Hausdorf error. Nevertheless, these methods fail
...
surface features like ridges and valleys, characerized bytheir first
...
features and this change is propagated towards other features, based
in the classified shapes and the inter-relationships between them.
This method works well with objects that have sharp edges, composed
of basic geometric shapes such as lines, circles or ellipses, but
the method is very limited at classifying when models are smooth since
it cannot find the features to edit and preserve.
...
inflation operations only depend on the normal vertex \cite{Stanculescu2011},
in grid-based sculpting, some other operations have allowed to add
or remove voxels, since producing polygonal meshes require processing
isosurfaces from volume \cite{Galyean1991}. The problem whit this
type of operations is the difficulty of maintaining the surface details
